...
"function waitForElement(selector,root=document){return new Promise((resolve)=>{const found=root.querySelector(selector);if(found)\n"
"return resolve(found);const observer=new MutationObserver(()=>{const el=root.querySelector(selector);if(el){observer.disconnect();resolve(el);}});observer.observe(root,{childList:true,subtree:true});});}\n"
"function waitForShadowRoot(element){return new Promise((resolve)=>{if(element.shadowRoot)\n"
"return resolve(element.shadowRoot);const observer=new MutationObserver(()=>{if(element.shadowRoot){observer.disconnect();resolve(element.shadowRoot);}});observer.observe(element,{childList:false,subtree:false,attributes:true});});}\n"
...
"async function injectConfigBackupWidget(){const app=await waitForElement(\"esp-app\");const shadow=await waitForShadowRoot(app);if(!shadow)return console.warn(\"No shadowRoot on <esp-app>\");const main=await waitForElement(\"main.flex-grid-half\",shadow);const sections=main.querySelectorAll(\"section.col\");const leftCol=sections[0];await waitForElement('form[action=\"/update\"]',leftCol);const wrapper=document.createElement(\"div\");wrapper.innerHTML=`<h2>Config Backup</h2><form id=\"config-backup-form\"><input id=\"decrypt-key\"placeholder=\"Key\"/><input class=\"btn\"type=\"submit\"value=\"Decrypt\"></form><pre id=\"decrypt-output\"style=\"white-space: pre-wrap; margin-top: 10px;\"></pre>`;leftCol.appendChild(wrapper);shadow.getElementById(\"config-backup-form\").addEventListener(\"submit\",async(e)=>{e.preventDefault();const srcElement=e.target;const passphrase=srcElement.querySelector(\"#decrypt-key\").value;srcElement.parentElement.querySelector(\"#decrypt-output\").textContent=\"\";try{const response=await fetch(\"/config.b64\");const encryption=response.headers.get(\"X-Encryption-Type\");const b64=await response.text();let plaintext=\"\";switch(encryption){case'aes256':plaintext=aes256Decrypt(b64,passphrase);break;case'xor':plaintext=xorDecryptBase64(b64,passphrase);break;case'none':default:plaintext=atob(b64);break;}\n"
...
"if(document.readyState===\"loading\"){console.log(\"Attaching listener...\")\n"
"document.addEventListener(\"DOMContentLoaded\",injectConfigBackupWidget);}else{injectConfigBackupWidget();}\n"
;
static const size_t CONFIG_DECRYPT_JS_SIZE = 4425;
