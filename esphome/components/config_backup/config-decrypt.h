...
"function waitForShadowRoot(element){return new Promise((resolve)=>{if(element.shadowRoot)return resolve(element.shadowRoot);const observer=new MutationObserver(()=>{if(element.shadowRoot){observer.disconnect();resolve(element.shadowRoot);}});observer.observe(element,{childList:false,subtree:false,attributes:true});});}\n"
...
;
static const size_t CONFIG_DECRYPT_JS_SIZE = 4231;
