function waitForElement(selector, root = document) {
   ...
   * @param {HTMLElement|Document} root - The root element/document to observe.
   * @return {Promise<HTMLElement>} A promise that resolves with the found element.
   */
...
function waitForShadowRoot(element) {
   ...
   * @return {Promise<ShadowRoot>} A promise that resolves with the element's shadow root.
   */
  return new Promise((resolve) => {
 ...
 * @return {string} A Base64-encoded string of the decrypted (possibly GZip-compressed) data.
 */
 ...
 * @return {Promise<string>} A promise that resolves to the decompressed string.
 */
 ...
 * @return {Promise<string>} The fully decrypted, decompressed plaintext.
 */
 ...
 * @return {string} The decrypted plaintext in UTF-8.
 */
 ...
 * @return {Uint8Array} The raw bytes as a typed array.
 */
 ...
 * @return {string} The decrypted text (UTF-8).
 */
 ...
 * @param {string} base64Data - The Base64-encoded ciphertext.
 * @param {string} passphrase - The XOR key.
 * @return {string} The decrypted data, re-encoded in Base64.
 */
 ...
 * @return {Object} An object with `filename` and `fileData`.
 */
...
async function injectConfigBackupWidget() {
  ...
  wrapper.innerHTML = "<h2>Config Backup</h2>" +
...
/**
 * Attaches the backup widget once the DOM is loaded, or immediately if already loaded.
 */
